...
| Версия | Нет | `1.0` | |
| Выпуск | Нет | `n2026` | |
| Имя сопровождающего | Нет | `$USER` | Предопределяются через `PKG_MAINTAINER_NAME` |
| Почта сопровождающего | Нет | `$USER@$HOSTNAME` | Предопределяются через `PKG_MAINTAINER_EMAIL` |
...
# Создать мета-пакет (нацелен на группу пакетов, без исходников)
nhopkg-src --init myapp --meta
```

С `--meta` инструмент создаёт **мета-пакет**: пакет без файлов исходников,
который лишь зависит от пакетов целевой группы. Метаданные заполняются
автоматически (`License: CUSTOM`, `Arch: any`, URL и описание выводятся из
имени), запрашивается `Target group name`, пакеты группы определяются из
метаданных репозитория и записываются как `# BuildDep:`/`# Dep(post):`, а
запросы Packageurl/хэш/VCS не задаются. Полученный проект упаковывается и
устанавливается обычным образом, но `nbuild()` остаётся `noemptyfuncs`, а при
установке создаётся `README.meta`.
...
### Предопределённые ответы через переменные окружения

Каждый вопрос `--init` можно ответить заранее, экспортировав соответствующую
переменную окружения. Если переменная уже установлена, `nhopkg-src` печатает
её значение и пропускает вопрос. Полный список переменных см. в английской
документации (`PKG_NAME`, `PKG_VERSION`, `PKG_RELEASE`,
...
Имя и почта сопровождающего задаются отдельно (`PKG_MAINTAINER_NAME` и
`PKG_MAINTAINER_EMAIL`); почта по умолчанию — `$USER@$HOSTNAME` (fallback
`user@host`). В nhoid они записываются одним полем
`# Package Maintainer: Name <email>`.

Пример — создать метапакет без единого вопроса:
...
| `# Packageurl:` | Ошибка | Должен присутствовать и быть непустым (не требуется для мета-пакетов) |
| `# Packageref:` | Ошибка | Обязательно, если Packageurl начинается с `git+` (не требуется для мета-пакетов) |
| `# SHA256:` | Предупреждение | Рекомендуется для tarball-исходников, не обязательно |
| Содержимое `nbuild()` | Ошибка | Должны быть реальные команды сборки (не только `noemptyfuncs`), кроме мета-пакетов |
| Содержимое `ninstall()` | Ошибка | Должны быть реальные команды установки (не только `noemptyfuncs`), кроме мета-пакетов |
